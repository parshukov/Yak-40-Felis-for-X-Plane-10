...
function update()
	...
	local XP10 = get(sim_version) == 10
	if XP10 and not message_printed then 
		print("engines tuned to XP10") 
...
if passed > 0 then
	...
	-- engine 1
	local throttle_1 = 0
	if XP10 then throttle_1 = interpolate(tro_table_10, rud1_last) * alt_coef * t_coef
	else throttle_1 = interpolate(tro_table_9, rud1_last) * alt_coef * t_coef end
	--smooth move
	...
	-- engine 2
	local throttle_2 = 0
	if XP10 then throttle_2 = interpolate(tro_table_10, rud2_last) * alt_coef * t_coef
	else throttle_2 = interpolate(tro_table_9, rud2_last) * alt_coef * t_coef end
	--smooth move
	...
	-- engine 3
	local throttle_3 = 0
	if XP10 then throttle_3 = interpolate(tro_table_10, rud3_last) * alt_coef * t_coef
	else throttle_3 = interpolate(tro_table_9, rud3_last) * alt_coef * t_coef end
	--smooth move
